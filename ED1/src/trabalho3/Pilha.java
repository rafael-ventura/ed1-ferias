...
public class Pilha<T> implements IPilha<T> {
	...
	//retorna baseado na importancia dos operadores
	public int precedencia(char caract) 
	{ 
		switch (caract) 
		{ 
		case '+': 
	...
	public String posFix(String expression){
		String result = "";
		...
		for (int i = 0; i <expression.length() ; i++) {
			...
			//checa se é operador
			if(precedencia(c)>0){
				while(pilha.estaVazia()==false && precedencia(pilha.peek())>=precedencia(c)){
					result += pilha.pop();
				}
				pilha.push(c);
			}else if(c==')'){
